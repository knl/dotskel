...
layers configuration. You are free to put any user code."
  ...
  ;; I like my lines broken at 80th char
  ...
  (when (display-graphic-p)
    ...
    (add-to-list 'default-frame-alist
                 (cons 'height (/ (- (x-display-pixel-height) 40)
                                  (frame-char-height))))
    (add-to-list 'default-frame-alist
                 (cons 'top 0)))
  ...
  ;; remove annoying dired message
  (setq dired-use-ls-dired nil)

  ;; this is to prevent opening new files in new frames
