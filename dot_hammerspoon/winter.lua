...
function CoordTrans:set(win, screen, f, _keep_proportions)
  ...
  if _keep_proportions then
    newf = {
      x = (f.x * screenrect.w // f.screenw) + screenrect.x,
      y = (f.y * screenrect.h // f.screenh) + screenrect.y,
      w = f.w * screenrect.w // f.screenw,
      h = f.h * screenrect.h // f.screenh,
    }
  else
    newf = {
      x = f.x + screenrect.x,
...
local function init_winter_action(title)
  return {
    ...
    _topmost = false,
    _bottommost = false,
    -- retain proportions, for example, when switching screens
    _proportional = false,
  }
end
...
--- mjolnir.winter:keep_proportions()
--- Function
--- Keep proportions
function WinterAction:keep_proportions()
...
function WinterAction:act()
  return function()
    ...
    while dscreen < 0 do
      ...
      dscreen = dscreen + 1
      print("Going to the previous screen" .. screen:name())
    end
    while dscreen > 0 do
      screen = screen:next()
      dscreen = dscreen - 1
      print("Going to the next screen" .. screen:name())
    end
    ...
    print("Now, I think I'm on " .. screen:name())
    ...
    f.screenw = origf.screenw
    f.screenh = origf.screenh
    ...
    self.ct:set(win, screen, f, self.keep_proportions)
  end
end
