...
This function should only modify configuration layer settings."
  (setq-default
   ...
   dotspacemacs-additional-packages '(direnv nimbus-theme)
...
before packages are loaded."
  ...
  ;; subtree.
  (defconst *my-journal-headline-format* "%A, %Y-%m-%d")
  (defun knl/find-todays-journal-point ()
    "Find or create the current project's header and insert the todo template"
    (goto-char (point-min))
    (let ((todo-header (concat "* " (format-time-string *my-journal-headline-format*))))
  ...
  (setq org-capture-templates
        '(("t" "Todo"
          ...
          ("j" "Journal Entry"
           plain
           (file+function "~/notes/journal.org" knl/find-todays-journal-point)
           "** [%<%H:%M>] %^{Title}\n %?"
           :empty-lines-after 1)
          ))
  ...
  (defun knl/new-buffer-frame ()
    "Create a new frame with a new empty buffer."
    (interactive)
    (let ((buffer (get-buffer-create "*scratch*")))
      (display-buffer buffer '(display-buffer-pop-up-frame . nil))))
  (spacemacs/set-leader-keys "Fn" 'knl/new-buffer-frame)
  )
